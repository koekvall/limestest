# devtools::install_github("koekvall/reconf@develop")
library(tictoc)
library(lme4)
...
psi_start <- psi_hat
fit_ours <- reconf::maximize_loglik(start_val = psi_hat,
...
psi_start[3] <- 0
fit_null_ours <- reconf::maximize_loglik(start_val = psi_start,
...
# Score test
stat <- reconf::score_stat(theta = fit_null_ours$arg,
                       test_idx = 3,
                       Y = getME(fit_lme4, "y"),
                       X = getME(fit_lme4, "X"),
                       Z = getME(fit_lme4, "Z"),
...
theta_hat <- c(fixef(fit_lme4_ml), reconf:::get_psi_hat_lmer(fit_lme4_ml))
theta_test <- theta_hat
theta_test[1] <- 1073
stat_values <- reconf::score_profile(theta_start = theta_test,
                                         test_idx = 1,
                                         max_radius = 150,
                                         num_points = 100,
                                         ...
                                         Hlist = reconf:::get_Hlist_lmer(fit_lme4_ml),
...
psi_null <- psi_hat
psi_null[3] <- 0
stat_values <- reconf::score_profile(theta_start = psi_null,
                          test_idx = 3,
                          max_radius = c(45, 10),
                          num_points = 1e2,
                          Y = getME(fit_lme4, "y"),
