...
test_that("C++ and R log-likelihood values agree at MLE (ML)", {
  ...
  ll_cpp <- limestest:::loglikelihood(
    psi = psi_hat, b = b_hat,
  ...
  Z     <- getME(fit, "Z")
  Psi_r <- limestest:::Psi_from_H_cpp(psi_mr = psi_hat[-r], H = H) / psi_hat[r]

  ll_R <- limestest:::loglik_psi(
    Z = Z,
...
test_that("C++ and R score vectors agree at MLE (REML)", {
  ...
  score_cpp <- limestest:::loglikelihood(
    psi = psi_hat,
    Y = Y, X = X, Z = Z, Hlist = Hlist, REML = TRUE,
    get_val = FALSE, get_score = TRUE, get_inf = FALSE
  )$score

  score_R <- limestest:::res_ll(
    XtX = crossprod(X), XtY = crossprod(X, Y),
    XtZ = crossprod(X, Z), ZtZ = crossprod(Z),
    YtZ = crossprod(Y, Z),
    Y = Y, X = X, Z = Z, H = H,
